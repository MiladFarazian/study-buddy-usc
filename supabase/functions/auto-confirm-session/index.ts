...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      logStep('ERROR: Failed to parse request body', { error: parseError instanceof Error ? parseError.message : 'Unknown error' });
    ...
    logStep('ERROR: Auto-confirm function failed', { error: errorMessage });
