-- Fix founding tutor badge and retroactively award badges to existing tutors
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE p.id = tutor_id AND p.created_at < timestamp with time zone '2025-10-01T00:00:00Z'
  ) THEN
    EXECUTE format('INSERT INTO public.tutor_badges (tutor_id, badge_type, criteria_met) 
