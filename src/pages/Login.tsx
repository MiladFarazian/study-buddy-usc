...
const Login = () => {
  ...
  // Check if this login page was opened specifically for iframe authentication
  const isIframeAuth = new URLSearchParams(location.search).get('iframe_auth') === 'true';
  ...
  useEffect(() => {
    ...
    // If this window was opened specifically for iframe auth, trigger the sign in automatically
    if (isIframeAuth && !user && !loading && !isSubmitting) {
      handleGoogleSignIn();
    }
  }, [from, isIframeAuth, user, loading]);

  if (user && !isIframeAuth) {
  ...
  // If this is an iframe auth window, show a simplified UI
  if (isIframeAuth) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center bg-slate-50">
        <div className="max-w-md w-full px-4">
          <Card>
            <CardHeader>
              <CardTitle>USC Authentication</CardTitle>
              <CardDescription>
                Please authenticate with your USC account to continue
              </CardDescription>
            </CardHeader>
