...
export const PaymentSettingsTab = () => {
  ...
  const { user, profile, isTutor, isStudent } = useAuth();
  ...
  // Check Stripe Connect account status for tutors
  useEffect(() => {
  ...
  const processTransfers = async () => {
    if (!user || !isTutor || !connectAccount?.has_account || connectAccount.needs_onboarding) return;
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          {isTutor ? renderTutorPaymentView() : renderStudentPaymentView()}
        </CardContent>
      </Card>
    </div>
  );
